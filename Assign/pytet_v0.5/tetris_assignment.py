...
class Tetris():
    ...
    def deleteFullLines(self):
        #print("deleteFullLines")
        for y in range(self.iScreenDy):
            self.iScreen=Matrix(self.oScreen)
            self.top=y
            self.left=self.iScreenDw
            isDelete=False
            self.tempBlk=self.iScreen.clip(self.top,self.left,self.top+1,self.left+self.iScreenDx)
            #self.tempBlk.print() 
            #print(self.tempBlk.sum())
            if self.tempBlk.sum()==self.iScreenDx:
                isDelete=True
            if isDelete==True:
                self.tempBlk=self.iScreen.clip(0, self.left, self.top, self.left+self.iScreenDx)
                self.oScreen=Matrix(self.iScreen)
...
'''    def deleteFullLines(self):
        ...
        for y in range(self.iScreenDy):
            ...
            isDelete=False
            for x in range(self.iScreenDx):
                if arr[y][x+self.iScreenDw]!=0:
                    isOkay+=1
            if isOkay==iScreenDx:
                isDelete=True
            if isDelete==True:
                self.tempBlk=self.iScreen.clip(0, self.left, self.top, self.iScreenDx-1)
                self.oScreen=Matrix(self.iScreen)
                self.oScreen.paste(self.tempBlk, 1, self.left)
        return self.state'''
